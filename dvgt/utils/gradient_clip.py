...
def check_gradient_norms(model: nn.Module, norm_type: int = 2):
    grad_norms = []
    print(f"--- 正在检查梯度 (Norm Type: {norm_type}) ---")
    for name, p in model.named_parameters():
        if p.grad is not None:
            param_norm = p.grad.norm(p=norm_type) / p.numel()
            grad_norms.append((name, param_norm.item()))

    grad_norms.sort(key=lambda x: x[1], reverse=True)

    for i, (name, norm) in enumerate(grad_norms[:10]):
        print(f"#{i+1}: {name:<50} | Norm: {norm:.4f}")
    print("------------------------------------")