    ...
    def infer(
        ...
        with torch.autocast(device_type=self.device.type, dtype=torch.float32):
            ...
            if points is not None:
                ...
                # NOTE: Focal here is the focal length relative to half the image diagonal，也就是说模型预测的f^ = f_pixel / (0.5 * d)，这个和归一化焦距 f_x / w类似
                # 这里用u=fx/(z+t)，来推导f和z_shift，是因为没有point_gt，无法用sp^+t-p来对齐
                if fov_x is None:
