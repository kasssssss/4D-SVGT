...
class RelativeEgoPoseLoss(nn.Module):
    ...
    def forward(self, predictions, batch):
        ...
        pred:
            relative_ego_pose_enc_list: List[B, T, 7]
        batch:
            ego_past_to_ego_curr: [B, T, 4, 4]
            point_masks: [B, T, V, H, W]
        """
        ego_pose_loss_dict = compute_ego_pose_loss(
            pred_pose_encodings=[pose_enc[:, 1:] for pose_enc in predictions['relative_ego_pose_enc_list']], 
            point_masks=batch['point_masks'][:, 1:], 
            gt_ego_pose=batch['ego_past_to_ego_curr'][:, 1:],
            prefix='relative_',
            T_loss_type=self.T_loss_type,
            R_loss_type=self.R_loss_type,
        )
        total_loss = ego_pose_loss_dict['loss_relative_ego_pose_T'] + ego_pose_loss_dict['loss_relative_ego_pose_R']
