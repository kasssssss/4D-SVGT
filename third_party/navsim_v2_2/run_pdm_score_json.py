...
cd /mnt/ad-alg/planning-users/xiezixun/vggt_work_dir/dvgt/third_party/navsim_v2_2
python run_pdm_score_json.py \
    --pth=/mnt/ad-alg/planning-users/xiezixun/vggt_work_dir/dvgt/output/stream_dvgt/eval/main_table/base_v3_1_openscene_navsim_e20/trajectory.json
"""

import argparse
import sys
parser = argparse.ArgumentParser()
parser.add_argument("--pth", type=str, default="/mnt/ad-alg/planning-users/xiezixun/vggt_work_dir/dvgt/output/stream_dvgt/eval/main_table/base_v3_openscene_navsim_e20/trajectory.json")
args = parser.parse_args()

args, unknown = parser.parse_known_args()
sys.argv = [sys.argv[0]] + unknown
TRAJ_PRED_PATH = args.pth
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    ...
    logger.info(f"Using {cfg.traffic_agents} traffic policy")
    ...
    with open(TRAJ_PRED_PATH, "r") as f:
        traj_pred = json.load(f)
    traj_pred_tokens = list(traj_pred.keys())

    tokens_to_evaluate = list(set(scene_loader.tokens) & set(metric_cache_loader.tokens) & set(traj_pred_tokens))
    missing_tokens = set(tokens_to_evaluate) - set(traj_pred_tokens)
    unused_tokens = set(traj_pred_tokens) - set(tokens_to_evaluate)
    tokens_to_evaluate = list(set(tokens_to_evaluate) & set(traj_pred_tokens))
    logger.info(f"{len(tokens_to_evaluate)} valid tokens, {len(missing_tokens)} missing tokens, {len(unused_tokens)} unused tokens")
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            trajectory = np.array(traj_pred[token][0], dtype=np.float32)[:8]
...
def main(cfg: DictConfig) -> None:
    ...
    """
    cfg.experiment_name = str(Path(TRAJ_PRED_PATH).parent / "navsim_v2_2")
    cfg.output_dir = str(Path(TRAJ_PRED_PATH).parent / "navsim_v2_2")
    ...
    metric_cache_loader = MetricCacheLoader(Path(cfg.metric_cache_path))
    print(f"use metric cache file: {cfg.metric_cache_path}")
