...
export MAX_JOBS=1
export MASTER_PORT=29763
export AUTO_RESUME=0
export RESET_OPTIMIZER=1
export LOG_DIR="data/nuscenes_dvgt_v0/logs/train_stage_c_gs_only_v63_online_dvgt_raydepth_anchor"

mkdir -p "$LOG_DIR"
bash tools/dvgt_occ/launch_train_ddp.sh \
  configs/dvgt_occ/train_stage_c_gs_only_v63_online_dvgt_raydepth_anchor.yaml \
  --log-interval 10 \
  --save-interval 25 \
  --val-interval 100 \
  --val-batches 1 \
