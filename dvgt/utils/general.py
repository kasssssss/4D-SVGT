...
def print_memory_usage(prefix=""):
    ...
    # 打印 RSS (Resident Set Size)，单位 GB
    print(f"[Rank {dist.get_rank()}] {prefix} Memory Usage: {mem_info.rss / (1024 ** 3):.2f} GB")
...
class ProgressMeter:
    def __init__(self, num_batches: int, meters, real_meters, prefix=""):
...
def set_seeds(seed_value, max_epochs=1, dist_rank=0):
    """
...
def merge_stream_outputs(outputs_list: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    在T维度，合并所有的输出结果。
    """
    ...
    for key in outputs_list[0].keys():
        if isinstance(outputs_list[0][key], torch.Tensor):
            final_outputs[key] = torch.stack([outputs[key][:, -1] for outputs in outputs_list], dim=1)
        elif isinstance(outputs_list[0][key], list):
            for idx in range(len(outputs_list[0][key])):
                final_outputs[key].append(torch.stack([outputs[key][idx][:, -1] for outputs in outputs_list], dim=1))
